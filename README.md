# @atomist/demo-sdm (pulumi)

an Atomist demo software delivery machine showcasing Pulumi integration

## Setup

### Prerequisites

Follow the [quick start guide](https://docs.atomist.com/quick-start/) to install Node.JS and NPM.

Additionally please make sure you have `kubectl` installed and a running Minikube cluster locally configured.

### Configuration

This SDM requires the following configuration to be set in `~/.atomist/client.config.json`:
...
```json
{
  "sdm": {
    "docker": {
      "hub": {
        "registry": "<the registry portion of the docker image name>",
        "user": "<your Docker Hub user name>",
        "password": "<your Docker Hub password>"
      }
    },
    "pulumi": {
      "token": "<your Pulumi access token>"
    }
  }
}
```
...
Now, you can start the SDM in local mode with `$ atomist start --local`.
...
and make some additional commits to the repo. Each commit will trigger the goal set and deploy the pod to the local
Minikube.
